...
Using the same datastructure for all YANG statements makes it possible to process all nodes of the module tree in the same way, which allows to generalize logic for various types of statements and ease the implementation.
...
For some module tree nodes the \texttt{info} field of the \texttt{\_YangNode} data stucture is initialized here. What data is stored in this field depends on the the type of the YANG statement:
...
\subsection{Import and include modules}
...
\end{verbatim}
\normalsize
function invocation. If a revision of the imported module is present then the implementation tries to load a module with the specified revision. If a module with the specified revision has not been found, the library attempts to load a module with no revision.
While loading imported modules (submodules) there is a validation that allows to detect circular dependencies between modules  (submodules).

\subsection{YANG list datastructures}
...
\end{verbatim}
\normalsize
which allow to avoid type casting.
