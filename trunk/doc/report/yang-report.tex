...
While loading imported modules (submodules) there is a validation that allows to detect circular dependencies between them.
...
The private API functions are defined in the \texttt{lib/yang-data.h} file.
\begin{itemize}
\item
\small
\begin{verbatim}
...
\end{verbatim}
\normalsize
returns a pointer to the first child node of the given node with the specified type.
...
\end{verbatim}
\normalsize
returns a pointer to the first child node of the given node with the specified type and argument value.
...
\end{verbatim}
\normalsize
resolves a reference to the node with the given type and argument value (\texttt{identifierName}) either in the current or imported module specified by the prefix.
